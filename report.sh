...
SESSIONS=$(docker exec -it "$1" bash -c "ls -ltd logs/* | grep 'dr' | grep -o -E 'logs.*' | sed 's/logs\///g'")

for session in $SESSIONS
do
    echo "Session: $session"
    RAW_PATH=$(echo "logs/$session"|sed 's/.$//') # newline removals
    REQUEST_FILES=$(docker exec -it "$1" bash -c "ls -t '$RAW_PATH' | sort")
    ...
    do
      ...
      then
        ...
        if [[ $request =~ "request" ]]; then
          ...
          TARGET=$(docker exec -it "$1" cat  $REQUEST_LOGFILE_PATH | grep -o -E "target\":\"[^\"]*" | sed "s/target\":\"//g")
          if [ ! -z $TARGET ]
          then
            echo "Target: $TARGET"
            IP=$(docker exec -it "$1" cat  $REQUEST_LOGFILE_PATH | grep -o -E "ip\":\"[^\"]*" | sed "s/ip\":\"//g")
            echo "Client IP: $IP"
          fi
        fi
        ...
        if [[ $request =~ "fileupload" ]]; then
          ...
          FILE=$(docker exec -it "$1" cat $FILE_UPLOAD_LOGFILE_PATH|grep -oe "file\":.*}}" | sed s/file\"://g)
          if [ ! -z $FILE ]
          then
            echo "Fileupload: $FILE"
            FILE_HASH=$(echo $FILE|grep -oe "hash\":.*}}" | sed s/hash\":\"//g | sed s/\"}}//g)
            mkdir -p ./takeouts/$1/
            docker cp $1:/var/www/html/$RAW_PATH/uploads/$FILE_HASH ./takeouts/$1/$FILE_HASH
          fi
        fi

        if [[ $request =~ "filedropnew" ]]; then
          FILE_DROP_LOGFILE_PATH=$(echo $request | sed 's/.$//g')
          FILE_DROP_NAME=$(docker exec -it "$1" cat  $RAW_PATH/$FILE_DROP_LOGFILE_PATH)
        ...
        if [[ $request =~ "dashboard" ]]; then
          ...
          TARGET_DASHBOARD_PAGE=$(docker exec -it "$1" cat  $RAW_PATH/$DASHBOARD_LOGFILE_PATH)
      ...
      if [[ $request =~ "credentials.json" ]]; then
        ...
        CREDENTIALS_USED=$(docker exec -it "$1" cat  $RAW_PATH/$CREDENTIALS_LOGFILE_PATH|grep -oe "credentials\":.*}}" | sed s/credentials\"://)
